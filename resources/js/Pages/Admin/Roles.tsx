        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {roles.map((rol) => (
            <Card key={rol.id} className="border-[#E5E7EB] hover:shadow-lg transition-all">
              ...
              <CardContent>
                <p className="text-sm font-semibold text-[#111827] mb-2">Permisos principales:</p>
                <div className="space-y-1">
                  {rol.permisos.slice(0, 4).map((permiso) => (
                    <div key={permiso.id} className="flex items-center gap-2 text-sm text-[#374151]">
                      <CheckCircle className="h-4 w-4 text-green-500 shrink-0" />
                      {permiso.nombre}
                    </div>
                  ))}
                  {rol.permisos.length > 4 && (
                    <p className="text-xs text-[#6B7280] pl-6">+{rol.permisos.length - 4} permisos más</p>
                  )}
                </div>
              </CardContent>
            </Card>
